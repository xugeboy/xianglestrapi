...
export default factories.createCoreController('api::product.product', ({ strapi }) => ({
  ...
  async getAttributeFilters(ctx: Context) {
    ...
    try {
      ...
      const attributeFields = [
        'length', 'width', 'material', 'finish', 'grade', 
        'working_load_limit', 'assembly_break_strength', 
        'webbing_break_strength', 'end_fitting', 'fixed_end_length',
        'ratchet_handle', 'product_weight'
      ];
  ...
  async filterProducts(ctx: Context) {
    try {
      const { categoryId, page = 1, pageSize = 10, ...attributeFilters } = ctx.query;
